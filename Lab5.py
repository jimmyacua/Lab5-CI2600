...
def train_discriminator(epochs, model, real, fake):
...
def train_generator(epochs, generador, discriminador, fake):
    ...
    error = loss_fn(prediction, ones)
    error.backward()
    # Update weights with gradients
    optimizer.step()
    # Return error
    print('Loss %f' % (float(error)))
    return error
...
if __name__ == '__main__':
    ...
    train_discriminator(100, discriminator, imagesTraining, fakes)

    generador = Generator()
    train_generator(10, generador, discriminator, fakes)
    '''model = Network()
